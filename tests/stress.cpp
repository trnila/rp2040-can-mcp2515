...
int can_open(const char *name) {
  ...
  // set buffer smaller than hardware one, otherwise we get ENOBUFS and POLLOUT will not work
  int sndbuf = 8;
...
int main(int argc, char** argv) {
  ...
  for(;;) {
    ...
    if(fds[1].revents & POLLIN) {
      ...
      stats_cur.rx++;
      auto found = std::find_if(frames.begin(), frames.end(), [&rd](const can_frame& f){
          return f.can_id == rd.can_id && f.can_dlc == rd.can_dlc && memcmp(f.data, rd.data, rd.can_dlc) == 0;
          });
    ...
    double diff = std::chrono::duration_cast<std::chrono::seconds>(now - last_stats).count();
    if(diff >= 1) {
      std::cout
        << "tx=" << (stats_cur.tx - stats_prev.tx) / diff << "/s "
